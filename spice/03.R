...
if (!interactive()) png("03.png", unit = "in", width = 7, height = 7, res = 500)
par(mar = c(4, 2, 1.5, 2))
...
dCT <- 0.5
for (CT in seq(CTpretty[1], tail(CTpretty, 1), dCT)) {
    lines(c(1, 1 + 0.6*gamma * D), rep(CTfcn(CT), 2), lwd = 0.6*lwdTickShort)
}
# text(1 + eps, 0.5, expression("Conservative Temperature [" * degree * "C]"), srt = 90)
text(0.95, 1 + 2.2 * D, "Conservative Temperature", pos = 3)
text(1, 1 + 0.6 * D, expression("[" * degree * "C]"), pos = 3)
...
# so far)
text(0.5, 1 + D, "Spiciness (at surface pressure)\n[g/kg]", pos = 3)
