...
Period <- function(k, h, g = 9.8) {
    ...
    sapply(seq_along(k), \(i) 2 * pi / sqrt(g * k[i] * tanh(k[i] * h[i])))
}
...
h <- seq(1, 200, length.out = n + 1) # diff lengths to check code
...
par(mfrow = c(2, 1), mar = c(3, 3, 1.5, 1), mgp = c(1.5, 0.45, 0))
contour(period, h, outer(period, h, CPperiod),
    labcex = 1, xaxs = "i", yaxs = "i", lwd = 2,
