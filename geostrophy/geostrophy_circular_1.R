...
R <- list(sigthe=0.65, S=0.8, lat=0.81)  # radii of axis circles
col <- list(sigmaTheta="royalblue", lat="darkred", S="darkgreen")
scale <- 2.0 * 2 * pi                        # adjust this to fill most of circle
cexName <- 1.25                        # cex for axis names

latfunc <- function(latitude)
{
    -scale*(log10(g/(L*oce::coriolis(latitude))))
}
...
for (layer in c("lower", "upper", "pointer")[1]) {
    if (!interactive())
