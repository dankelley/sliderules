...
\begin{equation}
\label{e2}
\sigma_\theta = \sigma_{\theta_0} + {\cal F}(S-S_0) + {\cal G}(T-T_0)
\end {equation}
where ${\cal F}$ and ${\cal G}$ are devised to approximate the full equation of
state over a reasonable application range.
...
an `r n`$^4$ grid covering the ranges $`r S0`<S<`r Smax`$, $`r T0`<T<`r Tmax`$
...
Run `sigma_theta_circular_.R` non-interactively, which creates files named
