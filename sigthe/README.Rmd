...
to $`r round(100*RMS(res01)/sigtheSpan,2)`$ percent of the $\sigma_\theta$
range. The maximum absolute misfit is $`r max(abs(res01))`$kg/m$^3$.
...
to $`r round(100*RMS(res02)/sigtheSpan,2)`$ percent of the $\sigma_\theta$
range. The maximum absolute misfit is $`r max(abs(res02))`$kg/m$^3$.
...
\begin{equation}
\label{e5}
\sigma_\theta = `r C03[1]` +`r C03[2]`(S-S_0) + `r C03[3]`(S-S_0)^2-`r -C03[4]`(T-T_0)-`r -C03[5]`(T-T_0)^2 -`r -C03[6]`(S-S_0)(T-T_0)
\end{equation}
with $S_0=`r S0`$, $T_0=`r T0`^\circ$C, at pressure $p=`r p`$dbar.  The RMS
misfit of this model is `r round(RMS(res03),3)` kg/m$^3$, which is equivalent
to $`r round(100*RMS(res03)/sigtheSpan,2)`$ percent of the $\sigma_\theta$
range. The maximum absolute misfit is $`r max(abs(res03))`$kg/m$^3$.
