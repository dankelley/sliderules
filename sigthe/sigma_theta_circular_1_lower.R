## Instructions: cut along the red circle; place this below sigma_theta_circular_1_upper.pdf
library(oce)
debug <- 0
lower <- TRUE
load("00.rda")
...
if (!interactive()) pdf("sigma_theta_circular_1_lower.pdf", width=7, height=7, pointsize=8)
nseg <- 512                            # segments in circle (128 is enough, but more is okay)
xc <- 0                                # centre x
yc <- 0                                # centre y
R <- list(sigthe=0.65, S=0.8, T=0.82)  # radii of axis circles
...
circle(R=1, col=2)                     # cut along this red circle
